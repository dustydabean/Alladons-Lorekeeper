...
class CollectionService extends Service {
    ...
    **********************************************************************************************/
    ...
    public function createCollection($data, $user) {
        ...
        try {
            ...
            if (!isset($data['rewardable_type'])) {
                throw new \Exception('Please add at least one reward to the collection.');
            }

            if (isset($data['collection_category_id']) && $data['collection_category_id'] == 'none') {
                $data['collection_category_id'] = null;
            }
            if ((isset($data['collection_category_id']) && $data['collection_category_id']) && !CollectionCategory::where('id', $data['collection_category_id'])->exists()) {
                throw new \Exception('The selected collection category is invalid.');
            }

            if (isset($data['parent_id']) && $data['parent_id'] == 'none') {
                $data['parent_id'] = null;
            }
            if ((isset($data['parent_id']) && $data['parent_id']) && !Collection::where('id', $data['parent_id'])->exists()) {
                throw new \Exception('The selected collection is invalid.');
            }
            ...
            if ($image) {
                $this->handleImage($image, $collection->imagePath, $collection->imageFileName);
            }
    ...
    public function updateCollection($collection, $data, $user) {
        ...
        try {
            ...
            if (Collection::where('name', $data['name'])->where('id', '!=', $collection->id)->exists()) {
    ...
    public function deleteCollection($collection) {
        ...
        try {
            ...
            if (DB::table('user_collections')->where('collection_id', $collection->id)->exists()) {
                throw new \Exception('At least one user currently owns this collection. Please remove the collection(s) before deleting it.');
            }
            if (Collection::where('parent_id', $collection->id)->exists()) {
                throw new \Exception('A collection currently has this collection as its parent.');
            }
    ...
    **********************************************************************************************/
     ...
     * @param User  $user
     *
     * @return bool|CollectionCategory
     */
    ...
    public function updateCollectionCategory($category, $data, $user) {
        ...
        try {
            // More specific validation
            if (CollectionCategory::where('name', $data['name'])->where('id', '!=', $category->id)->exists()) {
                throw new \Exception('The name has already been taken.');
            }
    ...
    public function deleteCollectionCategory($category) {
        ...
        try {
            ...
            if (Collection::where('collection_category_id', $category->id)->exists()) {
                throw new \Exception('A collection with this category exists. Please change its category first.');
            }
     ...
     *
     * @param array                   $data
     * @param CollectionCategory|null $category
     *
     * @return array
     */
