...
class CriterionService extends Service {
    ...
    */
    ...
    private function handleSort($sort, $model) {
        if (!$sort) {
            throw new \Exception('No criterion steps provided.');
        }
        $ids = explode(',', $sort);
        $steps = $model::whereIn('id', $ids)->orderByRaw(DB::raw('FIELD(id, '.implode(',', $ids).')')->getValue(DB::connection()->getQueryGrammar()))->get();
