...
class HuntService extends Service {
    ...
    */
    ...
    public function createHunt($data, $user) {
        ...
        try {
            ...
            if (!$data['end_at']) {
                throw new \Exception('An end time is required.');
            }
    ...
    public function updateHunt($hunt, $data, $user) {
        ...
        try {
            if (ScavengerHunt::where('name', $data['name'])->where('id', '!=', $hunt->id)->exists()) {
    ...
    public function deleteHunt($hunt) {
        ...
        try {
            ...
            if (HuntParticipant::where('hunt_id', $hunt->id)->exists()) {
                throw new \Exception('A user has participated in this hunt, so deleting it would break the logs. While hunts remain visible after their end time, they cannot be interacted with.');
            }
    ...
    **********************************************************************************************/
    ...
    public function createTarget($data, $user) {
        ...
        try {
            if (!$data['item_id']) {
                throw new \Exception('An item is required.');
            }
            if (!$data['quantity']) {
                throw new \Exception('A quantity is required.');
            }
            if (count(HuntTarget::where('hunt_id', $data['hunt_id'])->pluck('id')) == 10) {
                throw new \Exception('This hunt already has the maximum number of targets.');
            }

            $pageId = randomString(10);
            if (HuntTarget::where('page_id', $pageId)->exists()) {
                throw new \Exception('Failed to generate a unique page ID. Please try again!');
            }
    ...
    public function deleteTarget($target) {
        ...
        try {
            ...
            if (HuntParticipant::where('hunt_id', $hunt->id)->exists()) {
                throw new \Exception('A user has participated in this hunt, so deleting one of its targets would break the logs. While hunts remain visible after their end time, they cannot be interacted with.');
            }
