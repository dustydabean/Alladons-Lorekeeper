    ...
    */
    ...
    public function updateData($tag, $data) {
        ...
        try {
            ...
            if (!isset($data['rewardable_id']) && !isset($data['all_recipes'])) {
                return true;
            }
            if (isset($data['all_recipes'])) {
                $assets = ['all_recipes' => 1];
            } else {
    ...
    public function act($stacks, $user, $data) {
        ...
        try {
            ...
            if (!count($options)) {
                throw new \Exception('There are no more options for this recipe redemption item.');
            }
            if (count($options) < array_sum($data['quantities'])) {
                throw new \Exception('You have selected a quantity too high for the quantity of recipes you can unlock with this item.');
            }
