            ...
            foreach ($stacks as $key=>$stack) {
                ...
                if ((new InventoryManager)->debitStack($stack->user, 'Breeding Permission'.$stack->item->tag('breedingperm')->data == 1 ? '' : 's'.' Redeemed', ['data' => ''], $stack, $data['quantities'][$key])) {
                    for ($q = 0; $q < $data['quantities'][$key]; $q++) {
                        ...
                        if (!(new CurrencyManager)->creditCurrency($user, $character, 'Redeemed Breeding Permission'.($stack->item->tag('breedingperm')->data == 1 ? '' : 's'), 'Received from using '.$stack->item->name, Settings::get('breeding_permission_currency'), $stack->item->tag('breedingperm')->data)) {
                            throw new \Exception('Failed to redeem breeding permissions.');
                        }
                    }
                }
            }
