...
class ForageService extends Service {
    ...
    */
     ...
     * @param array $data
     *
     * @return \App\Models\Loot\Forage|bool
     */
    public function createForage($data) {
        ...
        try {
            ...
            if ($image) {
                $this->handleImage($image, $table->imagePath, $table->imageFileName);
            }
    ...
    public function initForage($id, $user) {
        ...
        try {
            ...
            if ($user->foraging->stamina < 1) {
                throw new \Exception('You have exhausted yourself already! Come back tomorrow.');
            }

            // check if a distribute_at already exists
            if ($user->foraging->forage_id && $user->foraging->distribute_at) {
                throw new \Exception('You have already begun foraging.');
            }
            ...
            if (!$forage->loot()->count()) {
                throw new \Exception('This forage has no rewards.');
            }
            if (config('lorekeeper.foraging.use_characters') && !$user->foraging->character_id) {
    ...
    public function claimReward($user) {
        ...
        try {
            ...
            if (!$forage) {
                throw new \Exception('Error finding forage.');
            }

            if (!$user->foraging->forage_id) {
                throw new \Exception('You have not started a forage yet.');
            }
            if (!$user->foraging->distribute_at) {
                throw new \Exception('You have not started a forage yet.');
            }
            if (!$user->foraging->forage->loot()->count()) {
                throw new \Exception('This forage has no rewards.');
            }
            ...
            if ($now->lt($distribute)) {
                throw new \Exception('You must wait until the forage is complete before claiming your rewards.');
            }
            ...
            if (!$rewards = fillUserAssets($rewards, $user, $user, $logType, $data)) {
            ...
            if (config('lorekeeper.foraging.use_characters')) {
                $name = $user->foraging->character->fullName;
            }
     ...
     *
     * @param mixed      $rewards
     * @param mixed|null $character_name
     */
