...
class CharacterLineageBlacklist extends Model {
    ...
    public static function getBlacklistCategories($level = '*') {
        return self::getIdList('category', $level);
    }
    ...
    public static function getBlacklistSpecies($level = '*') {
        return self::getIdList('species', $level);
    }
    ...
    public static function getBlacklistSubtypes($level = '*') {
        return self::getIdList('subtype', $level);
    }
    ...
    public static function getBlacklistRarities($level = '*') {
        return self::getIdList('rarity', $level);
    }
    ...
    public static function getBlacklistLevel($character, $maxLevel = 2) {
        ...
        foreach ($parts as $type => $id) {
            ...
            $entry = self::where('type', $type)->where('type_id', $id)->get()->first();
            if ($entry) {
                $level[$i] = $entry->complete_removal ? 2 : 1;
            }
    ...
    public static function getAncestorOptions($character = null) {
        $query = Character::myo(0)
            ->whereNotIn('character_category_id', self::getBlacklistCategories())
            ->whereNotIn('rarity_id', self::getBlacklistRarities())
    ...
    public static function searchAndSet($level, $type, $typeID) {
        $blacklistEntry = self::where('type', $type)->where('type_id', $typeID)->get()->first();
        $blacklist = false;
        if (isset($level)) {
            $blacklist = ($level == 1 || $level == 2);
        }
