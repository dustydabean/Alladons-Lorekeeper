                        ...
                        @foreach ($chunk as $featureId => $feature)
                            <div class="col-md-3 col-6 text-center align-self-center inventory-item">
                                @if ($feature->first()->has_image)
                                    <a class="badge" style="border-radius:.5em; {{ $feature->first()->rarity->color ? 'background-color: hsla(from #' . $feature->first()->rarity->color . ' h s l / 0.3);' : '' }}" href="{{ $feature->first()->url }}">
