...
{!! breadcrumbs(['Admin Panel' => 'admin', 'Dev Logs' => 'admin/devlogs', ($devLogs->id ? 'Edit' : 'Create').' Post' => $devLogs->id ? 'admin/devlogs/edit/'.$devLogs->id : 'admin/devlogs/create']) !!}
...
{!! Form::open(['url' => $devLogs->id ? 'admin/devlogs/edit/'.$devLogs->id : 'admin/devlogs/create', 'files' => true]) !!}
...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            {!! Form::text('post_at', $devLogs->post_at, ['class' => 'form-control datepicker']) !!}
        </div>
    </div>
</div>
...
$( document ).ready(function() {    
    $('.delete-logs-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/devlogs/delete') }}/{{ $devLogs->id }}", 'Delete Post');
    });
});
