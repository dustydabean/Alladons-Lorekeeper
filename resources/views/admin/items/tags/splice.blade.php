<p>
    You can choose if this splice item can be used on any companions (with variants) and all their variants, only specific companion species and all their variants, or only specific variants. While companions that are not visible will be shown here in the admin panel,
...
<div class="form-group">
    ...
    {!! Form::select(
        'splice_type',
        [
            'all' => 'Can be used on any companion for any variant',
            'by_species' => 'Can be used on specific companion(s) for all of their variants',
...
<div class="form-group by-species {{ isset($tag->getData()['splice_type']) && $tag->getData()['splice_type'] == 'by_species' ? '' : 'hide' }}">
    {!! Form::label('Select Companion Species') !!}
    {!! Form::select('parent_ids[]', $parents, isset($tag->getData()['parent_ids']) ? $tag->getData()['parent_ids'] : null, ['class' => 'form-control selectize', 'multiple', 'placeholder' => 'Select Companions']) !!}
</div>
