<div class="col p-1">
    <div class="border-bottom mb-1">
        ...
        @if ($max_depth == config('lorekeeper.lineage.lineage_depth') - 1)
            <a href="{{ $character ? $character->url : '#' }}" class="lineage-popover btn btn-sm btn-primaryline{{ $max_depth == config('lorekeeper.lineage.lineage_depth') - 1 ? ' h4' : '' }}" data-container="body" data-toggle="popover" data-content="{{ $character ? '<img src="' . $character->image->thumbnailUrl . '" class=\'img-thumbnail\' alt=\'Thumbnail for ' . $character->fullName . '\' style=\'width: 150px;\'>' : '' }}">
                {!! $character ? $character->fullName : 'Unknown' !!}
            </a>
        @else
            <a href="{{ $character ? $character->url : '#' }}" class="lineage-popover btn btn-sm btn-primaryline" data-container="body" data-toggle="popover" data-content="{{ $character ? '<img src="' . $character->image->thumbnailUrl . '" class=\'img-thumbnail\' alt=\'Thumbnail for ' . $character->fullName . '\' style=\'width: 150px;\'>' : '' }}">
                {!! $character ? $character->fullName : 'Unknown' !!}
            </a>
        @endif
    </div>
