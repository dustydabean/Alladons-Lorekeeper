    ...
    @if (config('lorekeeper.extensions.character_status_badges'))
        ...
        <div class="float-right">
            ...
            @if (!$character->is_myo_slot)
                ...
                <span class="btn {{ $character->is_links_open == 1 ? 'badge-success' : 'badge-danger' }} float-right ml-2" data-toggle="tooltip"
                    title="{{ $character->is_links_open == 1 ? 'OPEN for link requests.' : 'CLOSED for link requests.' }}"><i class="fas fa-link"></i></span>
            @endif
        </div>
    @endif
